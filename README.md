...
- **Usage-credit exhaustion**: Detects a loop that is authenticated and healthy but out of usage credits for the model it is running — every turn fails with `You're out of usage credits`, while every other detector reports a healthy session — corroborates it against the session transcript's turn-failure records, and injects `/model <target>` once to get it working again — including answering the "Switch model?" confirmation that command opens, since the switch does not happen until something does. Demote-only: it never switches back, and a confirmation left unanswered is reported as a failed demotion rather than a successful one. See [`docs/watchers.md`](docs/watchers.md)
